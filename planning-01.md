# Auth, Team Collaboration, and Realtime Chat Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a zero-budget, multi-user project workspace for TaskFlowy featuring Supabase email/password authentication, project ownership with invite codes/links, automated PostgreSQL task persistence, live Gantt synchronization, and an integrated realtime team chat drawer.

**Architecture:** The solution leverages Next.js 14 App Router connected to Supabase Free Tier (Auth, PostgreSQL, Row-Level Security, and WebSockets Realtime). Client and Server Component interactions use `@supabase/ssr` cookies for authentication. Projects use a project-centric model (`projects`, `project_members`, `tasks`, `project_messages`) with Row-Level Security enforcing strict tenant isolation. Gantt updates and team messages are broadcast live across connected clients via Supabase Realtime channels.
...
## Global Constraints & Security Protocols (Anti-SQLi & Hardening)
...
- **Database-Level CHECK Constraints:** All text columns must enforce strict length and regex limits directly in PostgreSQL:
  - `projects.name`: `1..100` characters.
  - `projects.invite_code`: Alphanumeric uppercase and hyphen only (`CHECK (invite_code ~ '^[A-Z0-9-]{6,12}$')`).
...
- **XSS & HTML Injection Prevention:** React automatic JSX string encoding prevents DOM execution; inputs are trimmed and stripped of non-printable ASCII control characters.
- **Row-Level Security (RLS) Enforcement:** Every table (`profiles`, `projects`, `project_members`, `tasks`, `project_messages`) has RLS enabled with explicit `auth.uid()` membership checks. No user can view, insert, or mutate data from projects they do not belong to.
...
├── supabase/
│   └── migrations/
│       └── 20260905_init_schema.sql         # DDL tables, CHECK constraints, RLS & search_path security
├── frontend/
...
│   │   ├── lib/
│   │   │   ├── validation/
│   │   │   │   └── schemas.ts               # Zod anti-injection & input sanitization schemas
│   │   │   ├── supabase/
│   │   │   │   ├── client.ts                # Browser client (createBrowserClient)
│   │   │   │   ├── server.ts                # Server component client (createServerClient)
│   │   │   │   └── middleware.ts            # Middleware cookie handler
│   │   │   └── services/
│   │   │       ├── project-service.ts       # Sanitized project CRUD & invite code operations
...
│   │       │   └── invite-modal.tsx         # Dual invite modal (copy link/code + email add)
│   │       └── chat/
...
- [ ] **Step 3: Create Zod Anti-Injection & Input Sanitization Schemas**
...
// Invite code: alphanumeric uppercase and hyphen only, 6-12 chars (e.g. FLOW-9X2A)
export const inviteCodeSchema = z
  .string()
  .trim()
  .toUpperCase()
  .regex(/^[A-Z0-9-]{6,12}$/, 'Invalid invite code format')
...
### Task 2: Hardened Database Schema DDL, RLS Policies & TypeScript Types
...
- Produces: `Database` TypeScript definitions, PostgreSQL tables: `profiles`, `projects`, `project_members`, `tasks`, `project_messages`.

- [ ] **Step 1: Write Hardened PostgreSQL DDL Migration with Strict Constraints and Anti-Hijacking `search_path`**
...
-- 2. Projects Table with strict format constraints
CREATE TABLE IF NOT EXISTS public.projects (
  ...
  CONSTRAINT chk_invite_code_format CHECK (invite_code ~ '^[A-Z0-9-]{6,12}$')
);
...
git commit -m "feat: add hardened supabase ddl migration with check constraints and rls policies"
```
...
export function AuthForm({ mode }: AuthFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate inputs with Zod before network submission
...
### Task 4: Project Dashboard & Project Creation (`/dashboard`) with Input Sanitization
...
- [ ] **Step 1: Write `project-service.ts` with parameter validation**
...
export async function joinProjectByInviteCode(code: string): Promise<Project> {
  const supabase = createClient()
  const { data: { user } } = await supabase.auth.getUser()
  ...
  const { data: project, error: pErr } = await supabase
    .from('projects')
    .select('*')
    .eq('invite_code', validatedCode)
    .single()

  if (pErr || !project) throw new Error('Invalid or expired project code.')
...
export function JoinProjectModal({ isOpen, onClose }: JoinProjectModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const project = await joinProjectByInviteCode(code)
...
git commit -m "feat: implement sanitized project dashboard, creation, and join operations"
```

---

### Task 5: Direct Join Link & In-App Invite Modal with Parameter Validation
...
**Interfaces:**
- Consumes: `inviteCodeSchema`, `uuidSchema` from `@/lib/validation/schemas`.
...
export default function JoinByCodePage() {
  ...
  useEffect(() => {
    async function handleJoin() {
      // Validate code format before attempting DB queries
      ...
      try {
        const project = await joinProjectByInviteCode(validation.data)
        router.push(`/projects/${project.id}`)
      } catch (err: unknown) {
...
- [ ] **Step 2: Create `InviteModal` for project workspace**
  ...
  const handleAddEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: profile, error: pErr } = await supabase
        .from('profiles')
        .select('id')
        .eq('email', emailResult.data.toLowerCase())
        .maybeSingle()
...
git commit -m "feat: implement validated invite flow preventing parameter tampering"
```

---

### Task 6: Database Task Service & Dynamic Workspace (`/projects/[id]`)
...
- Produces: Sanitized `fetchProjectTasks()`, `saveProjectTasks()`, `updateSingleTask()`.

- [ ] **Step 1: Write `task-service.ts` with validation and parameterized upsert**
...
export async function saveProjectTasks(projectId: string, tasks: TimelineTask[]): Promise<void> {
  ...
  // Validate every task record with Zod before database upsert
...
- [ ] **Step 3: Create `/projects/[id]/page.tsx` integrating Gantt, autosave, and dialogs**
...
export default function ProjectWorkspacePage() {
  ...
  // Debounced auto-save tasks when modified locally
...
```bash
git add frontend/src/lib/services/task-service.ts frontend/src/components/project/project-header.tsx frontend/src/app/projects/\[id\]
git commit -m "feat: implement validated task service and project workspace"
```
...
- Test: Anti-SQLi test suite probe & Antislop scan

- [ ] **Step 1: Write Anti-SQL Injection & Parameter Tampering Test Probe**
...
describe('Anti-SQLi & Schema Security Validation', () => {
  ...
  test('accepts valid alphanumeric invite codes', () => {
    expect(inviteCodeSchema.safeParse('FLOW-9X2A').success).toBe(true)
    expect(inviteCodeSchema.safeParse('TASK-ABCD').success).toBe(true)
  })
